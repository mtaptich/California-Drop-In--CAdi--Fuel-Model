...
def state_summary(s):
	# Set excel writer
	writer = pd.ExcelWriter(s.config.get('Paths', 'results_path')+'state_summary.xls')
	...
	df = pd.DataFrame(np.split(w, s.n_mid * s.n_down), columns=['Gasoline','Diesel']);
	df = df.sum().reset_index()
	df.columns = ['Type', 'Output (mt y-1)'] 
	df['Output (gal yr-1)'] = df['Output (mt y-1)'] / np.array([2.716e-03 , 3.395e-03])
	df['util'] = df['Output (mt y-1)'] / np.sum(s.d_data, axis=0)
	df.to_excel(writer,'Fuel Output', index=False)

	# Emissions
	data = []
	for e in ['GHG','NOx', "PM10", 'PM2.5', "SOx", "CO"]:
		costs, ech, ecp, ecUL = Z(s, e); 
		[ey, eX, ew, ev] = np.split(costs, s.blade);
		data.append({'Pollutant (kg y-1)': e, "Handling": round(np.dot(x , ech),0), "Upstream Transport": round(np.dot(x , ecUL),0), "Capital": round(np.dot(y , ey),0), "Fuel Production": round(np.dot(x , ecp),0), "Downstream Transport": round(np.dot(w , ew),0), "Total": round(np.dot(s.solution['set'] , costs),0) })
	pd.DataFrame(data).to_excel(writer,'Emissions Summary', columns=['Pollutant (kg y-1)', "Handling", "Upstream Transport", "Capital", "Fuel Production", "Downstream Transport", "Total"], index=False)

	# Freight
	df = up_freight(s);
	a = df.sum()[['cropres','forestres','scrapwood']].reset_index()
	a.columns = ['Feedstock', 'Consumption (mt -y)']
	a.to_excel(writer,'Feedstock Summary', index=False)

	df['rail'] = df['herb'] * df['herb_rail'] + df['wood'] * df['wood_rail'];
	df['road'] = df['herb'] * df['herb_road'] + df['wood'] * df['wood_road']
	df = df[['rail', 'road']].sum().reset_index()
	df.columns = ['mode', 'Turnover (tkm y-1)']
	df.to_excel(writer,'Freight Summary', index=False)
...
def store_at_mid(s):
	# Set excel writer
	writer = pd.ExcelWriter(s.config.get('Paths', 'results_path')+'feedstock_storage_at_refineries.xls')
	...
	feedstocks.columns = ['county', 'cropres_stock','forestres_stock', 'scrapwood_stock']
	...
	a = df.groupby(level=['county'])[['cropres','forestres', 'scrapwood']].sum().reset_index()
	a = pd.merge(a, feedstocks,on='county', how='left')
	a['cropres (%)'] = a['cropres'] / a['cropres_stock'];
	a['forestres (%)'] = a['forestres'] / a['forestres_stock'];
	a['scrapwood (%)'] = a['scrapwood'] / a['scrapwood_stock'];
	a.fillna(0, inplace=True)
	a.to_excel(writer,'Feedstock Utilization', index=False)


	df.groupby(level=['refinery']).sum().to_excel(writer,'Feedstock at Refinery')
	df.groupby(level=['refinery', 'pathway']).sum().to_excel(writer,'feedstock_pathway')
	a = df.groupby(level=['county', 'refinery']).sum() 
	a = a.loc[~(a==0).all(axis=1)]
	a.to_excel(writer,'upstream details')

	# Process Size
	df = pd.DataFrame(y, index=refineries, columns=['Units']).reset_index()
	df.columns = ['refinery', 'Units']
	df.to_excel(writer,'Process Size', index=False)

	#Storage
	df = pd.DataFrame(y*s.r_cap, index=refineries, columns=['mt yr-1']).reset_index()
	df.columns = ['refinery', 'mt yr-1']
	df.to_excel(writer,'storage (mt yr-1)', index=False)

	# Freight
	df = up_freight(s)
	df['rail'] = df['herb'] * df['herb_rail'] + df['wood'] * df['wood_rail'];
	df['road'] = df['herb'] * df['herb_road'] + df['wood'] * df['wood_road']
	df = df.groupby('destination')[['rail', 'road']].sum().reset_index()
	df.columns = ['refinery', 'rail, tkm y-1', 'road, tkm y-1']
	df.to_excel(writer,'freight (tkm y-1)', index=False)

	#Save
	writer.save()

def store_at_down(s):
	# Set excel writer and server connection
	writer = pd.ExcelWriter(s.config.get('Paths', 'results_path')+'fuel_storage_at_bulk_terminals.xls')
	server = Methods();
	...
	df.set_index(['refinery', 'terminal'], inplace=True)

	a = df.groupby(level=['terminal']).sum().reset_index()
	a['gas util. '] =  a['gas'] / a['terminal'].map(lambda x: s.d_data[x-1][0]);
	a['diesel util.'] =  a['diesel'] / a['terminal'].map(lambda x: s.d_data[x-1][1]);
	a.to_excel(writer,'Storage At terminal', index=False);

	# Downstream pathways
	df.groupby(level=['refinery']).sum().to_excel(writer,'at refinery')
	a = df.groupby(level=['refinery', 'terminal']).sum() 
	a = a.loc[~(a==0).all(axis=1)]
	a.to_excel(writer,'downstream details')

	# Send downstream logistics to server
	server.downstream_logistics(a.reset_index(), scenario=s.config.get('Extra', 'scenario')[0:2]);

	a = df.sum().reset_index()
	a.columns = ['Type', 'mt y-1']
	a['util'] = a['mt y-1'] / np.sum(s.d_data, axis=0)
	a.to_excel(writer,'total');

	# Freight
	df = df.reset_index()
	df.rename(columns={'refinery': 'origin', 'terminal': 'destination'}, inplace=True);
	t = pd.read_csv(s.config.get('Paths', 'extra_path')+'%s_%s_km.csv' % (s.config.get('Extra', 'scenario'), 'sjk')); 
	df = pd.merge(df, t, on=['origin', 'destination'], how='left')

	
	df['rail'] = (df['gas'] + df['diesel']) * df['km_rail'];
	df['road'] = (df['gas'] + df['diesel']) * df['km_road'];
	df['pipeline'] = (df['gas'] + df['diesel']) * df['km_pipe'];
	a = df.groupby('origin')[['pipeline','rail', 'road']].sum().reset_index()
	a.columns = ['refinery', 'pipeline', 'rail, tkm y-1', 'road, tkm y-1']
	a.to_excel(writer,'refinery (tkm y-1)', index=False)

	a = df.groupby('destination')[['pipeline','rail', 'road']].sum().reset_index()
	a.columns = ['terminal', 'pipeline', 'rail, tkm y-1', 'road, tkm y-1']
	a.to_excel(writer,'terminal (tkm y-1)', index=False)
...
def routes_used(s):
	...
	np.savetxt(s.config.get('Paths', 'extra_path')+'Zx.csv', x, delimiter=',')


class MeetTheTarget():
	def __init__(s, config_path='config/config_test_3x2x2x2.ini'):
	...
	def load(s):
		s.cropres_supply =  pd.read_csv(s.config.get('Extra', 'biomass_availability_origin_path'));
		s.forest_supply =  pd.read_csv(s.config.get('Extra', 'biomass_availability_origin_path'));
		s.scrapwood_supply =  pd.read_csv(s.config.get('Extra', 'biomass_availability_origin_path'));
		s.gas_production =  pd.DataFrame([])
		s.diesel_production =  pd.DataFrame([])
		s.gas_terminal =  pd.DataFrame([])
		s.diesel_terminal =  pd.DataFrame([])
		s.objective = [];

	def update(s, m):
		if m.trimmed_cols: 
			sset = np.zeros(m.var['x'] + m.var['y'] + m.var['w'] + m.var['v'] )
			np.put(sset, m.trimmed_cols, m.solution['set'])
			m.solution['set'] = sset

		[y, x, w, v] = np.split(m.solution['set'], m.blade)

		# Objective
		s.objective.append([m.target, m.solution['total']])

		# Biomass
		df = pd.DataFrame(np.split(x, m.n_up * m.n_mid * m.n_pathways), columns=['cropres','forestres', 'scrapwood'])
		refineries = np.genfromtxt(m.config.get('Extra', 'downstream_transport_origin_path'), skip_header=1)
		pathways = ['pg','m2g','pd','ft']

		df['pathway'] = df.index.map(lambda x: pathways[x % m.n_pathways])
		df['county'] = df.index.map(lambda x: x // m.n_pathways % m.n_up)
		df['refinery'] = df.index.map(lambda x: refineries[x // (m.n_up*m.n_pathways)])
		df.set_index(['county', 'refinery', 'pathway'], inplace=True);

		# Feedstock
		s.cropres_supply['t%i:1000' % int(1000*m.target)] = df.groupby(level=['county'])['cropres'].sum();
		s.forest_supply['t%i:1000' % int(1000*m.target)] =df.groupby(level=['county'])['forestres'].sum();
		s.scrapwood_supply['t%i:1000' % int(1000*m.target)] =df.groupby(level=['county'])['scrapwood'].sum();

		# Refineries and Fuel Terminals
		df = pd.DataFrame(np.split(w, m.n_mid * m.n_down), columns=['gas','diesel'])
		df['terminal'] = df.index.map(lambda x: x % (m.n_down) + 1)
		df['refinery'] = df.index.map(lambda x: refineries[x // (m.n_down)])
		df.set_index(['refinery', 'terminal'], inplace=True);

		# Refineries
		a = df.groupby(level=['refinery']).sum()
		s.gas_production['t%i:1000' % int(1000*m.target)] =  a['gas']
		s.diesel_production['t%i:1000' % int(1000*m.target)] =  a['diesel']

		#Terminals 
		a = df.groupby(level=['terminal']).sum()
		s.gas_terminal['t%i:1000' % int(1000*m.target)] =  a['gas']
		s.diesel_terminal['t%i:1000' % int(1000*m.target)] = a['diesel']

	def save(s):
		writer = pd.ExcelWriter(s.config.get('Paths', 'results_path')+'targeted_reductions_%s_results.xls' % s.config.get('Extra', 'emissions'));
		
		s.cropres_supply.to_excel(writer,'cropres_supply', index=False)
		s.forest_supply.to_excel(writer,'forest_supply', index=False)
		s.scrapwood_supply.to_excel(writer,'scrapwood_supply', index=False)
		
		s.gas_production.to_excel(writer,'gas_production')
		s.diesel_production.to_excel(writer,'diesel_production')
		
		s.gas_terminal.to_excel(writer,'gas_terminal')
		s.diesel_terminal.to_excel(writer,'diesel_terminal')
		
		pd.DataFrame(s.objective, columns=['Reduction Goal','kg CO2,e from baseline']).to_excel(writer,'objective', index=False)
